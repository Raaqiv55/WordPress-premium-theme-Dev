...
function _themename_customize_register($wp_customize){
    ...
    $wp_customize->add_control('_themename_portfolio_slug', array(
        'type'         =>  'text',
        'label'        => esc_html__('Portfoli Slug', '_themename'),
    ...
    $wp_customize->add_setting('_themename_site_info', array(
        'default' => '',
        'sanitize_callback' => '_themename_sanitiize_site_info',
        'transport' => 'postMessage'
    ));
...
function _themename_validate_footer_layout($validity, $value){
    if(!preg_match('/^([1-9]|1[012])(,([1-9]|1[012]))*$/', $value)){
        $validity->add('invalid_footer_layout', esc_html__('Footer layout is invalid', '_themaname'));
    }
    return $validity;
}

function _themename_sanitize_footer_bg($input){
    $value = array('light', 'dark');
...
function _themename_sanitiize_site_info($input){
