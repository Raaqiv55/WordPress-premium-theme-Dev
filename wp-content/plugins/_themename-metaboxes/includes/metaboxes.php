...
function _themename_post_metabox_html($post){
    ...
    ?>
    <p>
        <label for="_themename_post_metabox_html"><?php esc_html_e('Post Submit', '_themename-_pluginname'); ?></label>
        <br>
        <input class="widefat" type="text" name="_themeaname_post_subtitle_field" id="_themename_post_metabox_html" value="<?php echo esc_attr($subtitle); ?>">
    </p>
    <p>
...
function _themename_save_post_metabox($post_id, $post){
    ...
    if(array_key_exists('_themeaname_post_subtitle_field', $_POST)){
        update_post_meta(
            $post_id,
            '__themename_post_subtitle',
            sanitize_text_field($_POST['_themeaname_post_subtitle_field'])
        );
    }
